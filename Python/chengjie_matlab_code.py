...
# ★ 新增：金矿 KMZ 专属色阶接口
GOLD_YELLOW_THRESHOLD = 0.75  # 金矿导出时，亮黄色渐变对应的预测值起点 (0.0~1.0之间)
...
if len(sys.argv) > 2:
    ...
    # 默认路径备用
    data_path = r"C:\Users\Deep-Lei\Desktop\data\新疆高昌区库格孜觉北金矿-59.05km2【金】（四川黄金）（20260104任务，20260210下载）\data-矿权\RedEdge_Intrinsic_Result_gold_20260212_1419\gold_Result.mat"
...
if kmz_levels:
    ...
    for i, level in enumerate(cnt_kmz.levels):
        ...
        # ==================== ★ 定制 Gold 非线性色阶映射 (通过顶部接口控制) ★ ====================
        if mineral_type == 'gold':
            # 将你在顶部设置的 GOLD_YELLOW_THRESHOLD 锚定到 jet 色阶的 0.65 (黄色)
            if level >= GOLD_YELLOW_THRESHOLD:
                color_ratio = np.interp(level, [GOLD_YELLOW_THRESHOLD, 1.0], [0.65, 1.0])
            else:
                color_ratio = np.interp(level, [BASE_THRESHOLD, GOLD_YELLOW_THRESHOLD], [0.0, 0.65])
        else:
            # 其他矿种保持标准线性映射
