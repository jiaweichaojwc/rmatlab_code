...
if len(sys.argv) > 2:
    ...
    # 默认路径备用
    data_path = r"C:\Users\Deep-Lei\Desktop\新建文件夹\gold_Result.mat"
...
# ==================== ★ 新增：基于自适应马氏距离的矢量层 ★ ====================
fol_zones = kml.newfolder(name=f"高潜力区 (自适应马氏距离异常)")

# 1. 提取有效数据，屏蔽背景极小值和 NaN，避免大面积极低值拉低均值
valid_mask = (Au_deep_flip > np.nanmin(Au_deep_flip)) & (~np.isnan(Au_deep_flip))
valid_data = Au_deep_flip[valid_mask] if np.any(valid_mask) else Au_deep_flip

# 2. 计算统计基准
data_mean = np.nanmean(valid_data)
data_std = np.nanstd(valid_data)

# 3. 定义马氏距离阶梯 (对应 1D 的 Sigma 倍数)
m_distances = np.arange(1.0, 4.5, 0.5)

# 4. 反推真实的 Contour Levels
kmz_levels = []
for m_dist in m_distances:
    lvl = data_mean + m_dist * data_std
    if lvl >= KEY_AREA_THRESHOLD and lvl < np.nanmax(valid_data):
        kmz_levels.append(lvl)

# 5. 自适应保底机制
if len(kmz_levels) < 3:
    fallback_data = valid_data[valid_data >= KEY_AREA_THRESHOLD]
    if len(fallback_data) > 0:
        kmz_levels = np.percentile(fallback_data, [50, 75, 90, 95]).tolist()

kmz_levels = sorted(list(set(kmz_levels)))  # 去重并排序

if kmz_levels:
    print(f"--> [KMZ导出] 触发自适应马氏距离，生成的等值线级别为: {[round(x, 4) for x in kmz_levels]}")
    # 提取多边形路径，使用不可见的 alpha=0
    cnt_kmz = ax_kml.contour(utm_x_grid, utm_y_grid, Au_deep_flip, levels=kmz_levels, alpha=0)

    # 获取全局最大值，用于锚定颜色条的最顶端（确保颜色与底图对应）
    global_max = np.nanmax(valid_data)

    for i, level in enumerate(cnt_kmz.levels):
        paths = cnt_kmz.allsegs[i]

        # 核心修复：基于底图的全局阈值来计算颜色映射比例
        if global_max > BASE_THRESHOLD:
            color_ratio = (level - BASE_THRESHOLD) / (global_max - BASE_THRESHOLD)
        else:
            color_ratio = 1.0
        ...
        # 调整透明度为 77 (约45%)，防止多层嵌套时变成纯黑色
        hex_color_kml = f"77{b:02X}{g:02X}{r:02X}"

        for path in paths:
            if len(path) > 2:
                # 转换回经纬度
                lons, lats = transformer_utm2ll.transform(path[:, 0], path[:, 1])
                pol = fol_zones.newpolygon(name=f"异常边界: μ + {(level - data_mean) / data_std:.1f}σ",
                                           outerboundaryis=list(zip(lons, lats)))
                pol.style.polystyle.color = hex_color_kml
                pol.style.linestyle.color = 'FFFFFFFF'  # 白色线框增加辨识度
